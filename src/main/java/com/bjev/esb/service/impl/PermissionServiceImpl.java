...
import com.bjev.esb.mapper.localdb.sysbase.SysPermissionMapper;
...
public class PermissionServiceImpl extends ServiceImpl<SysPermissionMapper, SysPermission>
    ...
    /**
     * 根据用户查询拥有的权限 先查出用户拥有的角色 再去差用户拥有的权限 也可以多表关联查询
     */
    @Override
    public List<SysPermission> getPermission(String userId) {
        ...
        LambdaQueryWrapper<SysPermission> queryWrapper = Wrappers.<SysPermission>lambdaQuery()
                .in(SysPermission::getId, permissionIds).orderByAsc(SysPermission::getOrderNum);
        return sysPermissionMapper.selectList(queryWrapper);
    }

    /**
     * 删除菜单权限 判断是否 有角色关联 判断是否有子集
     */
    ...
    public void deleted(String permissionId) {
        ...
        // 获取下一级
        List<SysPermission> childs = sysPermissionMapper
                .selectList(Wrappers.<SysPermission>lambdaQuery().eq(SysPermission::getPid, permissionId));
        if (!childs.isEmpty()) {
        ...
        rolePermissionService
                .remove(Wrappers.<SysRolePermission>lambdaQuery().eq(SysRolePermission::getPermissionId, permissionId));
        // 刷新权限
    ...
    public List<SysPermission> selectAll() {
        List<SysPermission> result = sysPermissionMapper
                .selectList(Wrappers.<SysPermission>lambdaQuery().orderByAsc(SysPermission::getOrderNum));
        if (!result.isEmpty()) {
    ...
    /**
     * 获取所有的目录菜单树排除按钮 因为不管是新增或者修改 选择所属菜单目录的时候 都不可能选择到按钮 而且编辑的时候 所属目录不能 选择自己和它的子类
     */
    @Override

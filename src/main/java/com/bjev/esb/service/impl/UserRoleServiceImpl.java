...
import com.bjev.esb.mapper.localdb.sysbase.SysUserRoleMapper;
...
public class UserRoleServiceImpl extends ServiceImpl<SysUserRoleMapper, SysUserRole> implements UserRoleService {
  ...
  public List getRoleIdsByUserId(String userId) {
    LambdaQueryWrapper<SysUserRole> queryWrapper = Wrappers.<SysUserRole>lambdaQuery().select(SysUserRole::getRoleId)
  ...
  public List<String> getUserIdsByRoleId(String roleId) {
    List<Object> lists = sysUserRoleMapper.selectObjs(
        Wrappers.<SysUserRole>lambdaQuery().select(SysUserRole::getUserId).eq(SysUserRole::getRoleId, roleId));
    List<String> strs = (List<String>) (List) lists;
    return strs;
  }
}
